...
model.load_state_dict(torch.load("LSTM_callout_001.pth", map_location=device))
model.eval()
feat_scaler = joblib.load("scaler_callout_features_001.pkl")
tgt_scaler  = joblib.load("scaler_callout_target_001.pkl")
...
def _add_scaled_target_lags(df):
    df["callout_scaled"] = tgt_scaler.transform(df[["callout"]])
    for k in (1,2,3,4):
        df[f"lag{k}"] = df["callout_scaled"].shift(k)  # shift global, igual que en el entrenamiento
    return df
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        total_callout = float(pred.sum())

        next_tb = df["time_bin"].max() + pd.Timedelta(minutes=10)

        return {"status":"success",
                "predicted_total_callout": total_callout,
...
if __name__ == "__main__":
    import uvicorn; uvicorn.run(app, host="192.168.1.39", port=8001)
