...
Ejemplo de integración de LightGBM (como herramienta de ingeniería de características) + LSTM (en PyTorch) para predecir el tráfico de enlace de subida (smsout).
Descripción de las secciones del script:
1. Carga el dataset 'dataset_smsout.csv', ordena cronológicamente por fecha, hora, minuto y CellID. Normaliza los datos de 'smsout' con MinMaxScaler y genera características cíclicas (hour_sin, hour_cos, weekday_sin, weekday_cos) 
   y lags (retardos desde lag1 a lag4) a partir de 'smsout_scaled'.
2. Importa un modelo LightGBM preentrenado ('LightGBM_smsout_XXX.pkl'), selecciona las 8 características más relevantes y las normaliza con otro MinMaxScaler. Exporta ambos escaladores para ser utilizados en el servicio predictor.
3. Se crean secuencias temporales de 10 minutos (valor óptimo: seq_len=24) usando las características normalizadas y 'smsout_scaled' como objetivo (target).
...
Asunciones:
- Requiere Python >= 3.13, PyTorch con GPU, y archivos 'LightGBM_smsout_XXX.pkl' y 'dataset_smsout.csv' en la misma raíz del directorio del script.
"""
...
# 1.2 Normalizar el objetivo (target) 'smsout' con la función MinMaxScaler
target_scaler = MinMaxScaler()
df['smsout_scaled'] = target_scaler.fit_transform(df[['smsout']])
...
# Usar 'smsout_scaled' para generar los lags, como bien se hizo durante el entrenamiento de LightGBM
df['lag1'] = df['smsout_scaled'].shift(1)
df['lag2'] = df['smsout_scaled'].shift(2)
df['lag3'] = df['smsout_scaled'].shift(3)  # Se ha añadido lag3
df['lag4'] = df['smsout_scaled'].shift(4)  # Se ha añadido lag4
df.dropna(inplace=True)

# -------------------------------------------------------------------------
# 2. IMPORTAR LightGBM_smsout.pkl
# -------------------------------------------------------------------------
print("\nCargando modelo LightGBM (ingeniería de características)...")
lightgbm_model = joblib.load("LightGBM_smsout_001.pkl")
...
df = df[['date', 'hour', 'minute', 'CellID', 'weekday', 'idx', 'smsout', 'smsout_scaled'] + important_feats].copy()
df.dropna(inplace=True)
...
dump(scaler, "scaler_smsout_features_001.pkl")
dump(target_scaler, "scaler_smsout_target_001.pkl")
print("\nNormalización de las características de entrenamiento. Exportado como: scaler_smsout_features_001.pkl")
print("Normalización del target 'smsout'. Exportado como: scaler_smsout_target_001.pkl")
...
# 3.2 Convertir las características más importantes a un array y obtener el objetivo de interés (objetivo: smsout_scaled)
data_array = df[important_feats].values
target_array = df['smsout_scaled'].values  # Usar smsout_scaled como objetivo en la predicción
...
plt.xlabel('EPOCA')
plt.ylabel('MSE')
plt.title('Evolución del error cometido por el modelo LSTM smsout durante el entrenamiento')
plt.legend()
plt.grid(True)
plt.savefig('evolucion_MSE_smsout_001.png', dpi=900, bbox_inches='tight')
plt.close()
print("Gráfica de evolución del loss guardada como 'evolucion_MSE_smsout_001.png'.")

# -------------------------------------------------------------------------
# 6.9 EXPORTAR EL MODELO LSTM
# -------------------------------------------------------------------------
torch.save(model.state_dict(), "LSTM_smsout_001.pth")
print("\nModelo LSTM entrenado y exportado como 'LSTM_smsout_001.pth'.")
...
# 7.4 (Opcional) Calcular media y rango de smsout en prueba para el contexto de MAE y RMSE
mean_smsout = np.mean(y_test_seq)
min_smsout = np.min(y_test_seq)
max_smsout = np.max(y_test_seq)
...
print(f"\n--- Para las estadísticas de smsout en Prueba (escala normalizada) ---")
...
#print(f"Rango de smsout: [{min_smsout:.8f}, {max_smsout:.8f}]") 
...
axs[1].set_title(f"Tráfico real vs. predicción LSTM smsout \nSMAPE zoom: {smape_zoom:.2f}%", fontsize=12)

plt.tight_layout()
plt.savefig("Integracion_LightGBM_LSTM_smsout_001.png", dpi=900, bbox_inches='tight')
plt.close()

print("Figura 'Integracion_LightGBM_LSTM_smsout_001.png' creada.")
