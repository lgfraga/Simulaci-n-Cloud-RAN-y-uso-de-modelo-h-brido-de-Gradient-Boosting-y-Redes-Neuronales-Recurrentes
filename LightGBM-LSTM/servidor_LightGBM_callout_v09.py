...
MODEL_PATH  = "LightGBM_callout_001.pkl"
SCALER_PATH = "scaler_callout_001.pkl"
...
def _build_rows_next_interval(df2int):
    ...
    df2int["callout_scaled"] = scaler.transform(df2int[["callout"]])
    ...
    try:
        df = pd.read_csv(
            ...
            dtype={"date":str,"hour":int,"minute":int,
                   "CellID":int,"callout":float,"weekday":int,"idx":int}
        )
        df = _time_bin(df)
...
if __name__ == "__main__":
    import uvicorn; uvicorn.run(app, host="192.168.1.39", port=8000)