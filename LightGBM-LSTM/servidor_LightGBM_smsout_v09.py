...
MODEL_PATH  = "LightGBM_smsout_001.pkl"
SCALER_PATH = "scaler_smsout_001.pkl"
...
def _build_rows_next_interval(df2int):
    ...
    df2int["smsout_scaled"] = scaler.transform(df2int[["smsout"]])
    ...
    try:
        df = pd.read_csv(
            ...
            dtype={"date":str,"hour":int,"minute":int,
                   "CellID":int,"smsout":float,"weekday":int,"idx":int}
        )
        df = _time_bin(df)
...
if __name__ == "__main__":
    import uvicorn; uvicorn.run(app, host="192.168.1.39", port=8002)
