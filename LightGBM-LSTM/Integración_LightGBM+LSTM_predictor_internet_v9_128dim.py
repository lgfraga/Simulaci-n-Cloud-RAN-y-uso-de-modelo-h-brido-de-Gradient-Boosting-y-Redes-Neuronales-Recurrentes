...
Ejemplo de integración de LightGBM (como herramienta de ingeniería de características) + LSTM (en PyTorch) para predecir el tráfico de enlace de subida (internet).
Descripción de las secciones del script:
1. Carga el dataset 'dataset_internet.csv', ordena cronológicamente por fecha, hora, minuto y CellID. Normaliza los datos de 'internet' con MinMaxScaler y genera características cíclicas (hour_sin, hour_cos, weekday_sin, weekday_cos) 
   y lags (retardos desde lag1 a lag4) a partir de 'internet_scaled'.
2. Importa un modelo LightGBM preentrenado ('LightGBM_internet_XXX.pkl'), selecciona las 8 características más relevantes y las normaliza con otro MinMaxScaler. Exporta ambos escaladores para ser utilizados en el servicio predictor.
3. Se crean secuencias temporales de 10 minutos (valor óptimo: seq_len=24) usando las características normalizadas y 'internet_scaled' como objetivo (target).
...
Asunciones:
- Requiere Python >= 3.13, PyTorch con GPU, y archivos 'LightGBM_internet_XXX.pkl' y 'dataset_internet.csv' en la misma raíz del directorio del script.
"""
...
print(f"Total de filas en dataset_internet.csv: {len(df)}")

# 1.1 Ordenar el dataset temporalmente
df.sort_values(by=['date', 'hour', 'minute', 'CellID'], inplace=True)
df.reset_index(drop=True, inplace=True)

# 1.2 Normalizar el objetivo (target) 'internet' con la función MinMaxScaler
target_scaler = MinMaxScaler()
df['internet_scaled'] = target_scaler.fit_transform(df[['internet']])
...
# Usar 'internet_scaled' para generar los lags, como bien se hizo durante el entrenamiento de LightGBM
df['lag1'] = df['internet_scaled'].shift(1)
df['lag2'] = df['internet_scaled'].shift(2)
df['lag3'] = df['internet_scaled'].shift(3)  # Se ha añadido lag3
df['lag4'] = df['internet_scaled'].shift(4)  # Se ha añadido lag4
df.dropna(inplace=True)

# -------------------------------------------------------------------------
# 2. IMPORTAR LightGBM_internet.pkl
# -------------------------------------------------------------------------
print("\nCargando modelo LightGBM (ingeniería de características)...")
lightgbm_model = joblib.load("LightGBM_internet_001.pkl")
...
df = df[['date', 'hour', 'minute', 'CellID', 'weekday', 'idx', 'internet', 'internet_scaled'] + important_feats].copy()
df.dropna(inplace=True)
...
dump(scaler, "scaler_internet_features_001.pkl")
dump(target_scaler, "scaler_internet_target_001.pkl")
print("\nNormalización de las características de entrenamiento. Exportado como: scaler_internet_features_001.pkl")
print("Normalización del target 'internet'. Exportado como: scaler_internet_target_001.pkl")
...
# 3.2 Convertir las características más importantes a un array y obtener el objetivo de interés (objetivo: internet_scaled)
data_array = df[important_feats].values
target_array = df['internet_scaled'].values  # Usar internet_scaled como objetivo en la predicción
...
plt.xlabel('EPOCA')
plt.ylabel('MSE')
plt.title('Evolución del error cometido por el modelo LSTM internet durante el entrenamiento')
plt.legend()
plt.grid(True)
plt.savefig('evolucion_MSE_internet_001.png', dpi=900, bbox_inches='tight')
plt.close()
print("Gráfica de evolución del loss guardada como 'evolucion_MSE_internet_001.png'.")

# -------------------------------------------------------------------------
# 6.9 EXPORTAR EL MODELO LSTM
# -------------------------------------------------------------------------
torch.save(model.state_dict(), "LSTM_internet_001.pth")
print("\nModelo LSTM entrenado y exportado como 'LSTM_internet_001.pth'.")
...
# 7.4 (Opcional) Calcular media y rango de internet en prueba para el contexto de MAE y RMSE
mean_internet = np.mean(y_test_seq)
min_internet = np.min(y_test_seq)
max_internet = np.max(y_test_seq)
...
print(f"\n--- Para las estadísticas de internet en Prueba (escala normalizada) ---")
print(f"Media de internet: {mean_internet:.8f}")
#Se comenta la salida del reango pues no es de interés en la escala normalizada, se deja la implementación.
#print(f"Rango de internet: [{min_internet:.8f}, {max_internet:.8f}]") 
...
y_real = df_plot['internet'].values
...
axs[0].set_ylabel("Total de internet", fontsize=10)
...
axs[1].set_ylabel("Total de internet", fontsize=10)
axs[1].set_title(f"Tráfico real vs. predicción LSTM internet \nSMAPE zoom: {smape_zoom:.2f}%", fontsize=12)

plt.tight_layout()
plt.savefig("Integracion_LightGBM_LSTM_internet_001.png", dpi=900, bbox_inches='tight')
plt.close()

print("Figura 'Integracion_LightGBM_LSTM_internet_001.png' creada.")
