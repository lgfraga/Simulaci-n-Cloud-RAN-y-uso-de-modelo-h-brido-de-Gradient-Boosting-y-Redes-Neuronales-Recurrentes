...
# Agrupar por 'time_bin' y sumar el campo "smsout" en cada intervalo de 10 minutos.
smsout_sum = df.groupby("time_bin")["smsout"].sum()

# Obtener el valor máximo y mínimo, y sus respectivas marcas de tiempo.
max_smsout = smsout_sum.max()
min_smsout = smsout_sum.min()

time_of_max = smsout_sum.idxmax()  # Marca de tiempo donde se alcanza el valor máximo.
time_of_min = smsout_sum.idxmin()  # Marca de tiempo donde se alcanza el valor mínimo.

# Mostrar los resultados, formateando la marca de tiempo al formato deseado.
print("Valor máximo del total de smsout en intervalos de 10 minutos:", max_smsout)
print("Se produjo en:", time_of_max.strftime("%Y-%m-%d-%H-%M"))

print("\nValor mínimo del total de smsout en intervalos de 10 minutos:", min_smsout)
