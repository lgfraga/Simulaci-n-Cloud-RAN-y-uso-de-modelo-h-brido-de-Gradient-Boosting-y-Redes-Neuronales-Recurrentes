...
# 1. CARGAR EL DATASET
# -------------------------------------------------------------------------
# Esta sección carga el dataset desde el archivo "dataset_callout.csv" y lo ordena temporalmente 
...
# Además, se incorporan lags (retardos) de la variable objetivo que es 'callout' (específicamente, lag1 y lag2), 
# lo que permite al modelo considerar los valores pasados de la serie temporal para mejorar las predicciones.

# -------------------------------------------------------------------------
# 3. DEFINIR X e Y
# -------------------------------------------------------------------------
# Se define la variable objetivo 'Y' como la columna 'callout', que es el valor que se desea predecir. 
...
# superponiendo los tramos de entrenamiento, validación y prueba junto con los valores reales de 'callout'.
...
# -------------------------------------------------------------------------
# 2. NORMALIZAR LOS DATOS DE 'callout'
# -------------------------------------------------------------------------
scaler = MinMaxScaler()
df['callout_scaled'] = scaler.fit_transform(df[['callout']])
...
joblib.dump(best_model, "LightGBM_callout_001.pkl")
print("\nSerializando y exportando el modelo entrenado: LightGBM_callout_001.pkl ...")

# Exportar el escalador después de ajustarlo a los datos de entrenamiento
joblib.dump(scaler, "scaler_callout_001.pkl")
print("Escalador exportado como 'scaler_callout_001.pkl'.")
...
# 8.4 Agrupar 'callout' y cada columna de pred (en escala original)
...
plt.tight_layout()
plt.savefig("Regresion_LightGBM_callout_001.png", format='png', dpi=330, bbox_inches='tight')
plt.close()

print("Figura 'Regresion_LightGBM_callout_001.png' creada.")
