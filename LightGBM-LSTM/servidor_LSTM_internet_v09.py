...
model.load_state_dict(torch.load("LSTM_internet_001.pth", map_location=device))
model.eval()
feat_scaler = joblib.load("scaler_internet_features_001.pkl")
tgt_scaler  = joblib.load("scaler_internet_target_001.pkl")
...
def _add_scaled_target_lags(df):
    df["internet_scaled"] = tgt_scaler.transform(df[["internet"]])
    for k in (1,2,3,4):
        df[f"lag{k}"] = df["internet_scaled"].shift(k)  # shift global, igual que en entrenamiento
    return df
...
if __name__ == "__main__":
    import uvicorn; uvicorn.run(app, host="192.168.1.39", port=8005)
