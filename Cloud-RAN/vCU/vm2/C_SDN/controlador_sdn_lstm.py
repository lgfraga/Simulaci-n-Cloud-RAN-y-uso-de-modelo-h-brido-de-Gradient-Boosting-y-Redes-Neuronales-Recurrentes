...
def scale_prediction(pred_value: float, traffic_class: str) -> float:
    ...
    # Escalar el tráfico a 18 Mbit/s y redondear al valor más cercano
    scaled_value = ((pred_value - min_val) / (max_val - min_val)) * 18
    return round(max(0.0, scaled_value), 2)
...
def allocate_throughput(call_req: float, sms_req: float, net_req: float) -> Tuple[float, float, float]:
    ...
    call_lb = 0 #1 if 0 < call_req < 1 else 0  # 1 Mbit/s solo si 0 < call_req < 1, 0 si hay o no demanda
    ...
    call_ub = min(call_req, 18) # min(max(call_req, call_lb), 18)  # Asegura que call_ub >= call_lb y no exceda 18 (escalado de tráficos)
    ...
    # Obtener los valores óptimos y redondeados
    call_bw = 1 if 0 < call_req < 1 else round(x1.varValue) #(Para no cortar el tráfico si la demanda de callout es cercana a 0)
...
async def upload_datasets(
    ...
    print("En espera de los conjuntos de dataset del script de simulación...")
    files = {
    ...
    print("Datasets recibidos y guardados...")
...
def start_simulation(data: dict):
    ...
    # Step 3: Solicitud de métricas al agente SDN
    ...
    # Paso 5: Obtener predicciones (LSTM)
    predictions = {}
    ...
    if apply_response.status_code == 200:
        print("Configuración aplicada al OVS correctamente")
        results["bandwidth"][iteration] = config
        results["totals"][iteration] = totals

    # Paso 11: Limpieza temporal de los datasets
    for cls in ["callout", "smsout", "internet"]:
